...
Existuje  více způsobů, jak navrhnout architekturu síťové aplikce. Práce má zadanou podmínku navrhout prostředí v prostředí server-client. Proto je třeba důrazně rozlišit mezi danými způsoby implementace a zvolit ten odpovídající.
% Rozdíl mezi synchronním (Lockstep) a asynchronním (Autoritativní server) režimem.
\subsection{Asynchronní architektura}
Asynchronní architekura se řídí pravidlem, že je každý člen této architektury sebezodpovědný. V této architektuře je běžné, že je hostitelský proces pouze poskytovatel prvotních dat pro nově připojující se členy.
Nově připojující se proces si nejprve zašle požadavek o data od hostitelského procesu. Ten následně data odešle. Jelikož se jedná o kompletní balík se všemi herními daty, může se jednat i o poměrně veliký objem dat. Následně je
